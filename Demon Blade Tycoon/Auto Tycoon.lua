...
for _,Button in ipairs(BackgroundUI.Donations.Prices:GetChildren()) do
	if Button:IsA("TextButton") and Prices[Button.Name] then
		Button.MouseButton1Click:Connect(function()
			MarketplaceService:PromptProductPurchase(Player, Prices[Button.Name])
		end)
	end
end

local LockFrame = PlayerGui.Main.MorphMenu.LockFrame
LockFrame:GetPropertyChangedSignal("Visible"):Connect(function()
